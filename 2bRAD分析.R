...
library(ggplot2)
library(dplyr) 
genus<- read.csv("D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/Abundance.filtered_gt.anno.xls",sep="\t",header = T)
names(genus)
genus0<- genus[,c(6,10:48)] %>% as.data.frame()
names(genus0)
genus1 <- genus0 %>%
...
ggsave("D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/MPR_NMPR_2b/microeco/2b_MPR_Response2丰度堆砌图.pdf",p1,width = 9,height=8)
...
ggsave("D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/MPR_NMPR_2b/microeco/2b_物种箱状图.pdf",p3,width = 6,height=5)
...
p4<- genus_df$plot_heatmap(
  ...
  xtext_angle = 30)
p4
ggsave("D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/MPR_NMPR_2b/microeco/2b_heatmap.pdf",p4,width = 5,height = 7)



###物种丰度###
abundance<- read.csv("D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/Abundance.filtered_gt.anno.xls",sep="\t",header = T)
View(abundance)
colnames(abundance)

abundance_mat <- abundance[10:48]
tax_name <- abundance[1:7]
names(tax_name)
head(abundance_mat)
head(tax_name)

## 样本分组数据
group <- read.table("D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/MPR_NMPR_2b/group_mpr.csv", header=T, sep=",",row.names = 1, comment.char="",stringsAsFactors = TRUE,quote = "")
names(group)
...
write.csv(relative_abundance_aggregated,"D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/MPR_NMPR_2b/物种堆砌图/species_abundance.csv")
...
species_top50<- relative_abundance_aggregated[1:10,-ncol(relative_abundance_aggregated)]
...
palette <- distinctColorPalette(11) #差异明显的60种

# 2.1.3  物种组成堆叠柱形图-绝对丰度
library(ggplot2)

species_filter<-ggplot()+geom_bar(data=species_top50,
           aes(x=MPR_response2,
               weight=value,
...
ggsave("D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/MPR_NMPR_2b/物种堆砌图/2b_species_top10.pdf",species_filter,width =9,height =7)
...
write.csv(relative_abundance_aggregated,"D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/MPR_NMPR_2b/物种堆砌图/2b_genus_abundance.csv")

View(relative_abundance_aggregated)

#relative_abundance_aggregated<- relative_abundance_aggregated[rownames(relative_abundance_aggregated) != "Others", ]

genus_top50<- relative_abundance_aggregated[1:10,-ncol(relative_abundance_aggregated)]
...
new_data = genus_top50[genus_top50$group1=="ME"&genus_top50$value>0, ]
me_genus <- new_data$genus

genus_top50_filter<- genus_top50[-which(genus_top50$genus %in% me_genus),]

genus_top50_filter<-  genus_top50_filter[-which(genus_top50_filter$group1 == "ME"),]
View(genus_top50_filter)

###调整坐标轴位置
genus_top50$MPR_response2 <- factor(genus_top50$MPR_response2,levels = c("ME",'Pre_NMPR','Pre_MPR','Post_MPR'))
genus_top50_filter$MPR_response2 <- factor(genus_top50_filter$MPR_response2,levels = c('Pre_NMPR','Pre_MPR','Post_MPR'))
View(genus_top50)
# 2.1.2 颜色
library(ggsci)
col=pal_d3("category20")(20)
...
ggsave("D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/MPR_NMPR_2b/物种堆砌图/genus_abundance_top10.jpg",genus_filter,width =9,height =7)
ggsave("D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/MPR_NMPR_2b/物种堆砌图/genus_abundance_top10.pdf",genus_filter,width =9,height =7)
...
names(genus_top50)
library(dplyr)
data<-genus_top50 %>% filter(genus %in% c("Streptococcus", "Fusobacterium")) 
data$genus<- factor(data$genus,levels=c("Streptococcus", "Fusobacterium"))
data$MPR_response2<- factor(data$MPR_response2,levels=c("ME","Pre_NMPR","Pre_MPR","Post_MPR"))
...
pp<- ggplot(data) +
  geom_boxplot(aes(x = genus, y = value,fill=MPR_response2),outlier.shape = NA) +
  labs(title = "", x = "", y = "Relative abundance(%)") +  # 添加标题和轴标签
  scale_fill_manual(values = c("black","#c62d17", "#04d486","#f6c619"))+
  stat_pvalue_manual(df_p_val1,label="p.adj",hide.ns=T,
                     tip.length = 0,label.size = 5,color="black")+
  theme_minimal()+
  theme(
    panel.background = element_blank(),  # 去除面板背景
...
ggsave("D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/MPR_NMPR_2b/物种堆砌图/Streptococcus_Fusobacterium_boxplot.pdf",pp,width = 6,height = 5)
...
alpha<- read.csv("D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/MPR_NMPR_2b/alpha_diveristy/alpha_estimator_summary.xls",header = T,row.names = 1,sep="\t")
## 样本分组数据
...
alpha_MPR1<- alpha[match(rownames(group), rownames(alpha)),]
...
abundance_table<-  read.csv("D:/wangys_uestc_data2/OSCC_data/oscc_process/2bRAD_分析/MPR_NMPR_2b/Abundance.filtered.anno.csv", header=T, sep=",", row.names=1, stringsAsFactors = FALSE)
